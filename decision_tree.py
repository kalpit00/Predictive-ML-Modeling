...
class DecisionTree:
	def __init__(self, scorer, max_depth=2):
		"""
		...
		Args:
			scorer: which scorer to use for the splits (InfoGain or GiniGain)
	...
	def _build(self, x, y, max_depth, exclude=set()):
		...
		for feat_val, (x_subset, y_subset) in splits.items():
			node = self._build(x_subset, y_subset, max_depth-1, new_exclude)
			children[feat_val] = node
		## <<< END OF YOUR CODE <<<
		...
		if 'NA' not in children:
			## >>> YOUR CODE HERE >>>
			children['NA'] = LeafNode(self.scorer.compute_class_probs(self.scorer.class_labels))
			## <<< END OF YOUR CODE <<<
...
def load_restaurants_data(path: str):
    """
...
def evaluate_DecisionTrees():
	...
	"""
	X, y = load_restaurants_data(
		path.join(path.dirname(__file__), "dataset/restaurants_train.csv")
	)
	...
	scorer = InfoGain(class_labels=set(y))
	model = DecisionTree(scorer, max_depth=2)
	model.fit(X_train, y_train)
	
	loss, acc = decision_tree_performance(model, X_val, y_val)
	print(model)
	print("validation 0-1 Loss:", loss)
	print("validation accuracy: ", acc)
	print('\n------------------------------------------\n')


	## Plot the loss and accuracy figures vs. max_depth
	max_depths = list(range(1, 16)) # ranges from 1-10
	...
	# plot of accuracies
	fig2 = plt.figure()
	plt.plot(max_depths, train_accs, '-o', label='training')
	plt.plot(max_depths, val_accs, '-o', label='validation')
	plt.legend()
	plt.xlabel('max_depth')
	plt.ylabel('accuracy')
	plt.grid(True)
