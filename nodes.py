...
class LeafNode(TreeNode):
    ...
    def predict_proba(self, x):
        ...
        """
        ## >>> YOUR CODE HERE >>>
        probs = np.array([[self.probs[k] for k in sorted(self.probs)]])
        probs = probs.repeat(len(x), axis=0)
    ...
    def predict(self, x):
        ...
        predicted_labels = self.class_labels[predicted_labels]         ## <<< END OF YOUR CODE <<<
...
class SplitNode(TreeNode):
    ...
    def _choose_branch(self, x):
        ...
        for value in observed_values:
            idxs = (x[:, self.feature] == value).nonzero()[0]
            splits[value] = idxs        ## <<< END OF YOUR CODE <<<
